...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Load MCPs (fallback to hardcoded if file doesn't exist)
      const defaultMcps = [
      ...
      <div className="border-b border-slate-800 bg-slate-900/50 backdrop-blur sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold">Hermes Command Center</h1>
              <p className="text-xs text-slate-400">VEX OPCO Engine · Real-time Operations</p>
            </div>
          </div>
          <span className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full bg-green-500/10 text-green-400 border border-green-500/20">
                  ...
                  {agents.slice(0, 5).map((a: any, i: number) => (
                    <div key={i} className="flex items-center gap-3 p-3 rounded bg-slate-700/30">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-slate-400">{a.role || 'Unknown'}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm font-bold text-green-400">{Math.round((a.confidence || 0.5) * 100)}%</div>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <tr key={i} className="border-b border-slate-700/50">
                      ...
                      <td className="py-3 px-3">
                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold ${
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${a.status === 'active' ? 'bg-green-400' : 'bg-amber-400'}`}></span>
            ...
            {mcps.map((mcp: any, i: number) => (
              <div key={i} className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="font-bold text-sm">{mcp.name}</h3>
                    <p className="text-xs text-slate-400 mt-1">{mcp.category}</p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded ${
                    mcp.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-slate-600 text-slate-400'
                  }`}>
                    {mcp.status === 'active' ? '●' : '○'} {mcp.status}
                  </span>
                </div>
                <div className="text-xs text-slate-400">
                  <p className="mb-2">Capabilities: {mcp.capabilities?.join(', ') || '—'}</p>
                  <p>Used by: {mcp.used_by?.join(', ') || 'Internal'}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {skills.slice(0, 10).map((skill: any, i: number) => (
              <div key={i} className="bg-slate-800/50 border border-slate-700 rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold">{skill.name || skill.skill || 'Skill'}</h3>
                  <span className={`text-xs px-2 py-1 rounded font-mono ${
                    skill.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                    skill.status === 'running' ? 'bg-cyan-500/20 text-cyan-400' :
                ...
                <div className="h-2 bg-slate-600 rounded-full overflow-hidden mb-2">
                  <div className="h-full bg-gradient-to-r from-cyan-400 to-blue-400" style={{width: (skill.progress || 0) + '%'}}></div>
                </div>
                <div className="text-xs text-slate-400">
                  {skill.progress || 0}% · {skill.tokens_used || 0} tokens · ETA {skill.eta_seconds || 0}s
                </div>
              </div>
            ))}
          </div>

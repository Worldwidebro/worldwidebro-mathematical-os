...
} from '@realestate-os/shared-types';

export const AI_AGENTS_LIST: AgentMetadata[] = [
  {
...
export function getAllAgents(): AgentMetadata[] {
  return AI_AGENTS_LIST;
}

export function getAgentByName(name: string): AgentMetadata | undefined {
  return AI_AGENTS_LIST.find((agent) => agent.name.toLowerCase() === name.toLowerCase());
}

export async function invokeAgent(agentName: string, payload: any = {}): Promise<AgentExecutionResult> {

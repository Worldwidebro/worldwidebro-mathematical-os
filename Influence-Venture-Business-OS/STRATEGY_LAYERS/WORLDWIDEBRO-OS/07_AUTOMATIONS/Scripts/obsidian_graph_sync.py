...
from graph_entity_match import (
    ROOT as GRAPH_ROOT,
    alignment_status,
    build_repo_coverage_index,
    count_graph_entities,
    fetch_supabase_graph_entities,
    load_kg_venture_ids,
    load_merged_slug_index,
    refresh_planning_alignment_file,
...
ENRICHED_VENTURES_FILE = str(resolve_data_file("ventures_enriched_option_b.json", GRAPH_ROOT))
VENTURE_DEPENDENCIES_FILE = str(resolve_data_file("ventures_dependencies.json", GRAPH_ROOT))
BRIDGE_CSV = f"{OBSIDIAN_VAULT}/venture-hub/registries/venture_uuid_slug_bridge.csv"


def load_combined_by_uuid() -> Dict[str, dict]:
    path = f"{OBSIDIAN_VAULT}/venture-hub/docs/VENTURE_STARRED_OWNED_REPOS.csv"
...
def merge_combined_metadata(alignment_data: Dict[str, Any]) -> Dict[str, Any]:
    """Preserve owned/starred fields from align output (combined CSV)."""
    combined = load_combined_by_uuid()
    for v in alignment_data.get("ventures") or []:
        row = combined.get(v.get("venture_id", ""), {})
        if not row:
            continue
        v["owned_repo_slug"] = v.get("owned_repo_slug") or row.get("owned_repo_slug", "")
        found = (row.get("owned_repo_found_in_registry") or "").lower()
        v["owned_repo_found"] = found in ("yes", "true", "matched", "1")
        try:
            v["starred_repos_count"] = int(row.get("starred_repos_count") or 0)
        except (TypeError, ValueError):
            v["starred_repos_count"] = 0
...
def get_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    for env_file in (
        f"{OBSIDIAN_VAULT}/.env",
        f"{OBSIDIAN_VAULT}/.env.mcp.local",
        f"{OBSIDIAN_VAULT}/venture-hub/.env.mcp.local",
        f"{OBSIDIAN_VAULT}/venture-hub/.env.local",
    ):
...
def fetch_graph_data() -> Dict[str, Any]:
    """Fetch entities and relationships from Supabase (paginated entities)."""
    ...
    try:
        ...
        entities = fetch_supabase_graph_entities(SUPABASE_URL, SUPABASE_KEY)
        if not entities:
...
def load_cached_graph_data() -> Dict[str, Any]:
    """Use last successful export when Supabase is unavailable."""
    if not os.path.exists(GRAPH_DATA_FILE):
        return {"entities": [], "relationships": [], "status": "no_cache"}
    try:
        with open(GRAPH_DATA_FILE) as f:
            data = json.load(f)
        data["status"] = data.get("status") or "cached"
        data["synced_at"] = datetime.utcnow().isoformat()
        return data
    except Exception as e:
        return {"entities": [], "relationships": [], "status": f"cache_error: {e}"}
...
def build_alignment_report(graph_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not ventures and os.path.exists(ALIGNMENT_DATA_FILE):
        try:
            prior = load_local_json(ALIGNMENT_DATA_FILE, {})
            ventures = [
                {
                    "venture_id": v.get("venture_id"),
                    "venture_name": v.get("venture_name"),
                ...
                for v in (prior.get("ventures") or [])
                if v.get("venture_id")
            ]
        except Exception:
            pass
    if not ventures:
        prior = load_local_json(ALIGNMENT_DATA_FILE, {})
        if prior.get("ventures"):
            print(
                "WARNING: enriched ventures missing; keeping existing alignment "
                f"({len(prior['ventures'])} ventures)"
            )
            return prior
        raise RuntimeError(
            f"No ventures in {ENRICHED_VENTURES_FILE} and no prior alignment to restore"
        )
    ...
    repo_names_lower = build_repo_coverage_index(GRAPH_ROOT, supabase_repo_names)
    ...
    for venture in ventures:
        ...
        venture_slug = bridge.get("venture_slug") or (venture or {}).get("venture_slug") or ""
        ...
        matched_required_repos = [
            repo for repo in required_repos if required_repo_satisfied(repo, repo_names_lower)
        ]
        missing_required_repos = [
            repo for repo in required_repos if not required_repo_satisfied(repo, repo_names_lower)
        ]
        ...
        venture_record = {
            ...
            "owned_repo_found": (
                str(bridge.get("owned_gap_status", "")).lower() in ("matched", "yes")
                or str((venture or {}).get("owned_repo_found_in_registry", "")).lower()
                in ("yes", "true", "matched", "1")
                or bool(owned_slug)
            ),
            "starred_repos_count": int((venture or {}).get("starred_repos_count") or 0),
            ...
            "alignment_status": alignment_status(
                graph_connected, required_repos, repo_names_lower
            ),
        }
...
def main():
    ...
    local_only = "--local-only" in sys.argv or "--skip-fetch" in sys.argv

    if not SUPABASE_URL or not SUPABASE_KEY:
        if local_only or os.path.exists(GRAPH_DATA_FILE):
            print("⚠️  Supabase credentials not found — using cached graph-data.json")
            graph_data = load_cached_graph_data()
        else:
            ...
            print(f"\n3. Create .env file at: {OBSIDIAN_VAULT}/.env or venture-hub/.env.mcp.local")
            print(f"   Or run: python3 obsidian_graph_sync.py --local-only")
            sys.exit(1)
    else:
        graph_data = fetch_graph_data()
        if graph_data.get("status", "").startswith("error") or not graph_data.get("entities"):
            print("⚠️  Supabase fetch failed or empty — falling back to cached graph-data.json")
            graph_data = load_cached_graph_data()

    # Export to JSON
    filepath = export_to_obsidian(graph_data)
    prior = load_local_json(ALIGNMENT_DATA_FILE, {})
    prior_ventures = prior.get("ventures") or []
    use_refresh_only = len(prior_ventures) >= 100 and any(
        "starred_repos_count" in v for v in prior_ventures[:5]
    )

    if use_refresh_only:
        print("Using align-produced alignment; refreshing graph scores only")
        alignment_data = refresh_planning_alignment_file(
            Path(ALIGNMENT_DATA_FILE),
            graph_entities=graph_data.get("entities", []) or [],
            root=GRAPH_ROOT,
        )
        alignment_data = merge_combined_metadata(alignment_data)
    else:

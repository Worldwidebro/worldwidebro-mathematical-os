...
// Helper: Calculate tier-based allocation using CAPITAL-ALLOCATION.md formulas
...
async function calculateTierAllocation(
  supabase: any,
  opco_name: string,
  allocation_amount: number
): Promise<{ tier: string; calculated_amount: number; velocity_factor: number }> {
 ...
 */
capitalRoutingRouter.post('/allocate', async (req: Request, res: Response) => {
  try {
    const { opco_name, allocation_amount, week_start_date } = req.body as AllocationInput;
    ...
    const { tier, calculated_amount, velocity_factor } = await calculateTierAllocation(
      supabase,
      opco_name,
      allocation_amount
    );
    ...
    const response: CapitalDecision = {
      decision_id,
      opco_name,
      amount: calculated_amount,
      status: (approval_status === 'auto' ? 'approved' : 'pending') as 'approved' | 'pending' | 'rejected',
      approval_sla,
      confidence,
    };
 ...
 */
capitalRoutingRouter.post('/deploy', async (req: Request, res: Response) => {
  try {
    ...
    for (const venture_id of venture_ids) {
      ...
      console.log(
        `[CAPITAL-DEPLOY] ${decision.opco_name} → ${venture_id} ($${per_venture_amount.toFixed(2)})`
      );
    }
    ...
    try {
      ...
      execSync(
        `/Users/acebless/Documents/.fractal-venv/bin/python /Users/acebless/Documents/venture_loop.py --deploy '${JSON.stringify(loop_payload)}'`,
        { stdio: 'pipe' }
      );
 ...
 */
capitalRoutingRouter.get('/status/:decision_id', async (req: Request, res: Response) => {
  try {
 ...
 */
capitalRoutingRouter.post('/feedback', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      execSync(
        `/Users/acebless/Documents/.fractal-venv/bin/python /Users/acebless/Documents/01_loop_feedback_collector.py --feedback '${JSON.stringify(feedback_payload)}'`,
        { stdio: 'pipe' }
      );
 ...
 */
capitalRoutingRouter.get('/stats', async (req: Request, res: Response) => {
  try {
    const supabase = req.app.locals.supabase;
    ...
    res.json({
      ...
      summary: {
        total_decisions: slaMetrics?.reduce((sum: number, m: any) => sum + m.total_decisions, 0) || 0,
        auto_approved: slaMetrics?.reduce((sum: number, m: any) => sum + m.auto_approved, 0) || 0,
        total_deployed: roiReconciliation?.reduce((sum: number, d: any) => sum + (d.amount_deployed || 0), 0) || 0,
      },
    });
  } catch (err) {

...
founderRouter.get('/:id/human-os', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const tier_assignment = calculateTier(
      founder.execution_success_rate || 0,
      founder.learning_velocity || 0
    );
...
founderRouter.post('/:id/assessment', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const {

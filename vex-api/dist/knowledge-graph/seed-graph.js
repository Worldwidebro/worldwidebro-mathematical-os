export async function seedGraph(driver) {
    const session = driver.session();
    try {
        // Create organization
        await session.run(`MERGE (o:Organization {id: 'worldwidebro'})
       SET o.name = 'Worldwidebro Holdings', o.type = 'HOLDING'`);
        // Create OPCOs
        const opcos = ['CON', 'STA', 'FIN', 'RE', 'EDU', 'LOG'];
        for (const opco of opcos) {
            await session.run(`MERGE (o:Organization {id: $id})
         SET o.name = $name, o.type = 'OPCO'
         WITH o
         MATCH (parent:Organization {id: 'worldwidebro'})
         MERGE (o)-[:BELONGS_TO]->(parent)`, { id: opco + '-OS', name: opco + ' Operating System' });
        }
        // Create capabilities
        const capabilities = [

...
export class TaskMemoryStore {
     ...
     */
    async rememberTasksBatch(tasks) {
        if (!this.initialized) {
            throw new Error('TaskMemoryStore not initialized. Call .initialize() first');
        }
        const points = [];
     ...
     * Phase 3: Add caching
     */
    async findTasksByVenture(ventureId, content, limit = 10) {
        if (!this.initialized) {
            throw new Error('TaskMemoryStore not initialized. Call .initialize() first');
        }
        ...
        console.warn(`Unfiltered task listing not implemented. Use findSimilarTasks() with content.`);
        return [];
    }
    /**

...
networkRouter.get('/opportunities', async (req, res) => {
    try {
...
networkRouter.post('/opportunities/:id/accept', async (req, res) => {
    try {
        const { id } = req.params;
        const supabase = req.app.locals.supabase;
        const { data, error } = await supabase
...
networkRouter.get('/delegation', async (req, res) => {
    try {
        const supabase = req.app.locals.supabase;
        const { data, error } = await supabase
            .from('delegations')
...
networkRouter.get('/health', async (req, res) => {
    try {

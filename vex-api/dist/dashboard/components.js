...
export const TierBadge = ({ tier }) => {
    const getTierClass = (tierString) => {
        if (tierString.includes('AUTONOMOUS'))
            return 'autonomous';
        if (tierString.includes('MONITORED'))
            return 'monitored';
        return 'training';
    };
    const getTierIcon = (tierString) => {
        if (tierString.includes('AUTONOMOUS'))
            return '⚡';
        if (tierString.includes('MONITORED'))
            return '⚙️';
    ...
    return (<div className={`tier-badge ${tierClass}`}>
 ...
 */
export const LayerChart = ({ layers }) => {
    ...
    return (<section className="layers-section">
      ...
      <div className="layers-grid">
        {layers.map((layer) => {
            ...
            return (<article key={layer.layer} className="layer-card">
              <div className="layer-code">{layer.layer}</div>
              <div className="layer-name">{layer.name}</div>
              <div className="layer-bar">
                <div className={`layer-bar-fill ${scoreClass}`} style={{ width: `${percentage}%` }} role="progressbar" aria-valuenow={layer.score} aria-valuemin={0} aria-valuemax={10} aria-label={`${layer.name}: ${layer.score} out of 10`}/>
              </div>
 ...
 */
export const MetricsGauge = ({ executionRate, learningVelocity, }) => {
    const getExecutionClass = (rate) => {
        if (rate >= 90)
            return 'green';
        if (rate >= 70)
            return 'yellow';
    ...
    const getLearningClass = (velocity) => {
    ...
    const getLearningLabel = (velocity) => {
        if (velocity <= 48)
            return 'Fast';
        if (velocity <= 72)
            return 'Moderate';
    ...
    return (<section className="metrics-section">
      <div className="gauge">
        ...
        <div className="gauge-bar">
          <div className={`gauge-bar-fill ${executionClass}`} style={{ width: `${executionRate}%` }} role="progressbar" aria-valuenow={executionRate} aria-valuemin={0} aria-valuemax={100} aria-label={`Execution success rate: ${executionRate} percent`}/>
        </div>
      ...
      <div className="gauge">
        ...
        <div className="gauge-bar">
          <div className={`gauge-bar-fill ${learningClass}`} style={{ width: `${velocityPercentage}%` }} role="progressbar" aria-valuenow={Math.min(learningVelocity, 96)} aria-valuemin={0} aria-valuemax={96} aria-label={`Learning velocity: ${learningVelocity} hours`}/>
        </div>
        <div className="gauge-label">
 ...
 */
export const TrendTimeline = ({ assessments }) => {
    ...
    return (<section className="trend-section">
      ...
      <div className="trend-chart">
        <svg className="sparkline" viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label="Execution rate trend chart">
          {/* Grid lines */}
          <line x1="0" y1={yPadding} x2={chartWidth} y2={yPadding} stroke="#e9ecef" strokeWidth="1"/>
          <line x1="0" y1={chartHeight / 2} x2={chartWidth} y2={chartHeight / 2} stroke="#e9ecef" strokeWidth="1"/>
          <line x1="0" y1={chartHeight - yPadding} x2={chartWidth} y2={chartHeight - yPadding} stroke="#e9ecef" strokeWidth="1"/>

          {/* Trend line */}
          <polyline points={polylinePoints} stroke="#ffc107" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round"/>

          {/* Data points */}
          {points.map((point, index) => (<circle key={index} cx={point.x} cy={point.y} r="4" fill="#ffc107"/>))}
        </svg>
      </div>
      ...
      <div className="trend-labels">
        {assessments.map((assessment, index) => {
            ...
            return (<div key={index} className="trend-point">
 ...
 */
export const FounderDashboard = ({ data }) => {
    ...
    return (<div className="dashboard-container">
      ...
      <header>
        ...
        {data.last_assessment && (<p style={{ fontSize: '14px', color: '#999', marginTop: '8px' }}>
      ...
      {/* Metrics Gauges */}
      <MetricsGauge executionRate={data.execution_success_rate} learningVelocity={data.learning_velocity}/>
